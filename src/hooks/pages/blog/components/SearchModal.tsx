...
import { SearchModalProps } from '@/hooks/pages/blog/types/Blog';
    ...
    return (
        ...
        >
            <motion.div
                ...
                exit={{ opacity: 0, y: -50 }}
                className="modal-box max-w-3xl bg-background backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-[var(--border-color)]"
            >
                <div className="relative">
                    <input
                        ...
                        className="w-full pl-14 pr-6 py-4 text-lg bg-gray-50/50 rounded-2xl
                                 border border-[var(--border-color)] focus:outline-none focus:ring-2 focus:ring-blue-500/20
                ...
                {searchResults.length > 0 && (
                    <div className='mt-5'>
                        <h3 className="font-medium text-gray-500 mb-4 px-2">Hasil Pencarian</h3>
                        <div className="space-y-4 max-h-[60vh] overflow-y-auto px-2 custom-scrollbar">
                            {searchResults.map((article) => (
                                <div
                                    key={article.id}
                                    onClick={() => handleSearchResultClick(article)}
                                    className="flex items-start gap-4 p-4 rounded-2xl border border-[var(--border-color)] hover:bg-gray-50/80
