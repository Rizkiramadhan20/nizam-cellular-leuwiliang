...
import {
  ...
  BrandHandphoneContentFormData,
} from "@/hooks/dashboard/super-admins/handphone/brand/types/BrandHandphone";

export const useBrandHandphoneData = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [allContents, setAllContents] = useState<BrandHandphoneContent[]>([]);
  ...
  useEffect(() => {
    setIsLoading(true);
    const brandHandphoneRef = collection(
      db,
      process.env.NEXT_PUBLIC_COLLECTIONS_PRODUCT_BRAND_HANDPHONE as string
    );
    const q = query(brandHandphoneRef, orderBy("createdAt", "desc"));
  ...
  const createContent = async (formData: BrandHandphoneContentFormData) => {
    try {
