...
import { ContentModalProps } from '../types/Piutang';
...
export const ContentModal = forwardRef<HTMLDialogElement, ContentModalProps>(
  ({ isEditing, formData, setFormData, handleSubmit, isSubmitting, onClose }, ref) => {
            ...
            <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-6 md:p-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-gray-500">Isi informasi di bawah ini untuk {isEditing ? 'update' : 'tambah'} data piutang</p>
                </div>
                <button
                  onClick={onClose}
                        ...
                        <div className="flex gap-4">
                          <label className="inline-flex items-center">
                            <input
                              type="radio"
                              value="belum_bayar"
                              checked={formData.status === 'belum_bayar'}
                              onChange={(e) => setFormData({ ...formData, status: e.target.value as 'belum_bayar' | 'sudah_bayar' })}
                              className="w-4 h-4 text-indigo-600 border-gray-300 focus:ring-indigo-500"
                            />
                            <span className="ml-2 text-sm text-gray-700">Belum Bayar</span>
                          </label>
                          <label className="inline-flex items-center">
                            <input
                              type="radio"
                              value="sudah_bayar"
                              checked={formData.status === 'sudah_bayar'}
                              onChange={(e) => setFormData({ ...formData, status: e.target.value as 'belum_bayar' | 'sudah_bayar' })}
                              className="w-4 h-4 text-indigo-600 border-gray-300 focus:ring-indigo-500"
                            />
                            <span className="ml-2 text-sm text-gray-700">Sudah Bayar</span>
                          </label>
                        </div>
                      </div>
                    </div>
