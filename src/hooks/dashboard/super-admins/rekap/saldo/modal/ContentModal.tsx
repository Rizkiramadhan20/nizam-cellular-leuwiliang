...
import { ContentModalProps } from '@/hooks/dashboard/super-admins/rekap/saldo/types/Saldo';
            ...
            <div className="flex items-center justify-between mb-8">
              <div className="space-y-1">
                <h3 className="text-2xl font-bold text-gray-900">
                  {isEditing ? 'Edit Saldo' : 'Tambah Saldo'}
                </h3>
                <p className="text-sm text-gray-500">Isi informasi di bawah ini untuk {isEditing ? 'update' : 'tambah'} data saldo</p>
              </div>
              <button
                onClick={onClose}
                    ...
                    <div className="form-control">
                      ...
                      <div className="relative">
                        <input
                          type="text"
                          className="w-full px-4 py-2.5 rounded-xl border border-[var(--border-color)] focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200 bg-transparent"
                          value={formData.saldo ? FormatRupiah(formData.saldo) : ''}
                          onChange={(e) => {
