...
import { Article, Category, ArticleModalProps } from '@/hooks/dashboard/super-admins/blog/blog/types/Blog';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (article?.id) {
        ...
        await updateDoc(doc(db, process.env.NEXT_PUBLIC_COLLECTIONS_BLOG as string, article.id), articleData);
        onSuccess?.();
      } else {
        // Create new article
        await addDoc(collection(db, process.env.NEXT_PUBLIC_COLLECTIONS_BLOG as string), articleData);
        onSuccess?.();
      }
