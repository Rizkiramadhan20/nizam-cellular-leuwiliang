...
export const useIconsData = () => {
    ...
    const [contents, setContents] = useState<IconsContent[]>([]);
    ...
    const fetchContents = useCallback(async () => {
        try {
            ...
            const q = query(
                collection(db, process.env.NEXT_PUBLIC_COLLECTIONS_PRODUCT_ICONS as string),
    ...
    const handleImageUpload = useCallback(async (file: File): Promise<string> => {
        try {
            ...
            const result = await imagekitInstance.upload({
                file: base64,
                fileName: `icons-content-${Date.now()}`,
    ...
    const createContent = useCallback(async (formData: IconsContent, imageUrl: string) => {
        try {
    ...
    const handleUpdate = useCallback(async (id: string, formData: IconsContent) => {
        try {
            const docRef = doc(
                db,
                process.env.NEXT_PUBLIC_COLLECTIONS_PRODUCT_ICONS as string,
                id
            );
