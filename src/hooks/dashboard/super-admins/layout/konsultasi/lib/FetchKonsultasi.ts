...
let contentCache: KonsultasiContent[] | null = null;
let lastFetchTime: number | null = null;
const CACHE_DURATION = 5 * 60 * 1000; // 5 menit

export const useKonsultasiData = () => {
  const [isLoading, setIsLoading] = useState(true);
  const [contents, setContents] = useState<KonsultasiContent[]>([]);
  ...
  const fetchContents = useCallback(async (force = false) => {
    ...
    try {
      ...
      const q = query(
        collection(
          db,
          process.env.NEXT_PUBLIC_COLLECTIONS_KONSULTASI as string
        ),
  ...
  const handleImageUpload = async (file: File) => {
    try {
      ...
      const result = await imagekitInstance.upload({
        file: base64,
        fileName: `konsultasi-content-${Date.now()}`,
        folder: "/konsultasi-contents",
      });
  ...
  const createContent = async (data: KonsultasiContent, imageUrl: string) => {
    try {
  ...
  const handleUpdate = async (id: string, updatedData: KonsultasiContent) => {
    try {
      const docRef = doc(
        db,
        process.env.NEXT_PUBLIC_COLLECTIONS_KONSULTASI as string,
