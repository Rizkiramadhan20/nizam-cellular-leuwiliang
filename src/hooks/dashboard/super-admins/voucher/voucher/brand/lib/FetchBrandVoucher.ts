...
import {
  BrandVoucherContent,
  BrandVoucherContentFormData,
} from "@/hooks/dashboard/super-admins/voucher/voucher/brand/types/BrandVoucher";

export const useBrandVoucherData = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [allContents, setAllContents] = useState<BrandVoucherContent[]>([]);
  ...
  useEffect(() => {
    setIsLoading(true);
    const brandVoucherRef = collection(
      db,
      process.env.NEXT_PUBLIC_COLLECTIONS_BRAND_VOUCHER as string
    );
    const q = query(brandVoucherRef, orderBy("createdAt", "desc"));
  ...
  const createContent = async (formData: BrandVoucherContentFormData) => {
    try {
