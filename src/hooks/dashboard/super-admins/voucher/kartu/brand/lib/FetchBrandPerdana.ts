...
import {
  BrandPerdanaContent,
  BrandPerdanaContentFormData,
} from "@/hooks/dashboard/super-admins/voucher/kartu/brand/types/BrandPerdana";

export const useBrandPerdanaData = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [allContents, setAllContents] = useState<BrandPerdanaContent[]>([]);
  ...
  useEffect(() => {
    setIsLoading(true);
    const brandPerdanaRef = collection(
      db,
      process.env.NEXT_PUBLIC_COLLECTIONS_BRAND_PERDANA as string
    );
    const q = query(brandPerdanaRef, orderBy("createdAt", "desc"));
  ...
  const createContent = async (formData: BrandPerdanaContentFormData) => {
    try {
