...
let contentCache: ContactContent[] | null = null;
...
export const useKonsultasiData = () => {
  ...
  const handleImageUpload = async (file: File) => {
    try {
      ...
      const result = await imagekitInstance.upload({
        file: base64,
        fileName: `contact-content-${Date.now()}`,
        folder: "/contact-content",
      });
  ...
  const createContent = async (data: ContactContent, imageUrl: string) => {
    try {
  ...
  const handleUpdate = async (id: string, updatedData: ContactContent) => {
    try {
      const docRef = doc(
        db,
        process.env.NEXT_PUBLIC_COLLECTIONS_CONTACT as string,
        id
      );
